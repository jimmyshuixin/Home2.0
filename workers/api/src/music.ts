...
export function createMusicHandler(origin: string, fetcher: typeof fetch = globalThis.fetch.bind(globalThis)) {
  ...
  async function upstream(server: string, type: string, id: string): Promise<unknown> {
    ...
    const text = await boundedText(response, type === 'lrc' ? 200000 : 2 * 1024 * 1024);
    try { return JSON.parse(text) as unknown; }
    catch { return text; }
  }
  async function nativeQQUrl(songmid: string): Promise<string | undefined> {
    ...
    const response = await fetcher(url, { headers: { accept: 'application/json', referer: 'https://y.qq.com/' }, redirect: 'manual', signal: AbortSignal.timeout(15000) });
    if (!response.ok) await response.body?.cancel();
    assert(response.ok, 'MUSIC_UNAVAILABLE', 503, 'QQ 音乐暂时无法提供播放地址');
    let payload: unknown;
    try { payload = await readBoundedJson(response, 128 * 1024); } catch { throw new ApiError('MUSIC_UNAVAILABLE', 503, 'QQ 音乐返回的播放信息无效'); }
    ...
    for (const base of data.sip) {
      try {
        ...
        if (safe && (destination.hostname === 'qq.com' || destination.hostname.endsWith('.qq.com'))) return safe;
  ...
  return async (request: Request, snapshot: Snapshot, requestId: string): Promise<Response> => {
    ...
    const location = parsed.success ? publicMediaUrl(parsed.data.url) : playlist.source === 'tencent' && legacyQQDescriptor(resolved, track.songmid || track.id) ? await nativeQQUrl(track.songmid || track.id) : undefined;
