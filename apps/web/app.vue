...
<template><SiteEntrance /><a class="skip-link" href="#main-content">跳到正文</a><div aria-hidden="true" :class="['ink-backdrop', { 'interior-backdrop': route.path !== '/' }]" /><div :class="['wrap', 'ink-shell', { 'freepaper-shell': route.path === '/' }]"><header class="topbar"><NuxtLink class="brand" to="/" aria-label="虚宁，返回首页">虚宁</NuxtLink><button ref="menuButton" class="mobile-menu icon-button" :aria-expanded="menu" aria-controls="site-nav" :aria-label="menu ? '关闭导航' : '打开导航'" @click="menu = !menu"><SiteIcon :name="menu ? 'close' : 'menu'" /></button><nav id="site-nav" :class="{ 'is-open': menu }" aria-label="主导航"><NuxtLink v-for="item in nav" :key="item.href" :to="item.href" :class="{ 'home-active': route.path === '/' && item.href === '/about' }" :aria-current="route.path.startsWith(item.href) ? 'page' : undefined">{{ item.label }}</NuxtLink></nav></header><main id="main-content" tabindex="-1"><NuxtPage /></main><SiteFooter /></div><MusicEngine /></template>
