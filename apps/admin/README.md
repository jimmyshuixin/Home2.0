...
- 发布中心通过私有 `/admin/release-changes` 逐集合分页汇总已保存草稿；可跨创作、摄影、健身、歌单和设置勾选/全选待发布改动，单个版本最多50项，隐藏内容默认不选。清单绑定当前公开版本，版本变化必须重新读取；同一内容不重复加入。
- 编辑字段和媒体库提供图片放大、音频/视频原生预览；混排创作可在保存前预览当前编辑。只请求已处理的私有媒体版本，音视频 `preload=none`、点击才播放、共用播放协调器；不预下载大原件。元数据读取并发最多4个。外部平台内容提供明确的平台链接。
- 发布其他已保存内容时保留当前未保存编辑；会话过期时暂停播放、关闭顶层预览窗口并保留编辑，便于重新登录。
- 发布创建使用幂等UUID请求键；历史中的未确认构建可按已有任务ID重试。超过10项时继续分步同步后台记录，不把网站已上线误当作所有记录已同步。新候选不能覆盖创建后别处发布的新版本。
...
npm exec vitest run apps/admin/tests
