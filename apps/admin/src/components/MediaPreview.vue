...
<template>
  <div class="asset-preview" :class="{ compact }" :aria-label="name">
    ...
    <template v-else-if="source">
      ...
      <video v-else-if="asset.kind === 'video'" :key="source" ref="player" :src="source" :poster="poster" controls playsinline preload="none" :aria-label="`视频预览：${name}`" @play="started" @error="failed = true"><track v-if="captions" kind="captions" label="中文字幕" srclang="zh" :src="captions" default></video>
